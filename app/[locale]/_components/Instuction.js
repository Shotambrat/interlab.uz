      ...
      <div className='flex flex-col self-start max-md:max-w-full'>
        <div className='flex flex-col max-md:max-w-full'>
          <h2 className='text-3xl mdx:text-4xl font-bold text-neutral-900 max-md:max-w-full leading-7 max-w-[467px]'>
            ...
            {locale === 'ru' ? 'Инструкции по сдаче анализов' : 'Tahlillarni topshirish bo‘yicha ko‘rsatmalar'}
          </h2>
          ...
          <p className=' text-sm mdx:text-lg text-[#FB6A68] max-md:max-w-full leading-4'>
            ...
            {locale === 'ru' ? '*Клиника INTERMED не несет ответственности за корректность результатов анализов при несоблюдении данных правил' : 'INTERMED klinikasi ushbu qoidalarga rioya qilinmagan taqdirda tahlillar natijalarining aniqligi uchun javobgar emas.'}
                  ...
                  {item.description.map((des, index) => (
                    <div key={index} className='flex gap-3 max-md:flex-wrap'>
                      <img
                        ...
                        alt='Bullet point'
                      />
                      <p className='max-md:max-w-full'>{des.item[locale]}</p>
                    </div>
                  ))}
                </div>
              </AccordionContent>
